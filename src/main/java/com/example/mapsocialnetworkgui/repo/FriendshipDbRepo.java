...
//                LocalDateTime localDateTime = LocalDateTime.ofInstant(date.toInstant(), ZoneId.systemDefault());
                FriendshipStatus status = Objects.equals(resultSet.getString("status"), "accepted") ? FriendshipStatus.ACCEPTED : FriendshipStatus.PENDING;
                Friendship f = createFriendshipAndAddToUsers(id, idU1, idU2, date, status);
    ...
    public Optional<Friendship> save(Friendship entity) {
        ...
        String sql = "insert into \"friendships\" (id, u1, u2, date, status) values (DEFAULT, ?, ?, ?, ?)";
        ...
        try (Connection connection = DriverManager.getConnection(url, username, password);
            ...
            ps.setString(3, date);
            String stateString = entity.getStatus()==FriendshipStatus.ACCEPTED ? "accepted" : "pending";
            ps.setString(4, stateString);
            ps.executeUpdate();
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public Optional<Friendship> update(Friendship entity) {
        String sql = "update \"friendships\" set u1=?, u2=?, date=?, status=? where id=?";
     ...
     */
    public Friendship createFriendshipAndAddToUsers(Long id, Long idU1, Long idU2, LocalDateTime localDateTime, FriendshipStatus status){
        ...
        Friendship f = new Friendship(id,u1, u2, localDateTime, status);
